...
This is a capstone project for my journey of Google Data Analytics Professional Certificate. This project is about getting insights of how bike-sharing differ between casual users and annual members for Cyclistic Company. 
...
Data was made available and downloaded via Motivate International Inc. under this [liscense](https://ride.divvybikes.com/data-license-agreement). Data has coverage over the period from January 2020 to December 2020. The data was downloaded and unzipped as 10- .csv files, one presented the first quarter of 2020 and the rest - one for each month of the range. Original copies and 10 .csv files were saved as separate folders. I opened .csv files in Excel and saved them as Excel Workbook files, so I can perform analysis later. All files were saved in separate folders (.zip / .csv / .xls) properly. 

Notes: This is a public data that can be used to explore how different customer types are using Cyclistic bikes. But data-privacy issues prohibit users from using riders' personally identifiable information. This means no one will b able to connect pass purchases to credit card numbers to determine if casual riders live in Cyclistic service area or if they have purchased multiple single passes.
...
Data has covererage over the period from Janurary 2020 to December 2020. The data was downloaded and unzipped as 10- .csv files, one presented the first quarter of 2020 and the rest - one for each month of the range.
...
All columns names were checked and they were all the same. Aside of ensuring consistency of column names, I also checked the internal structure of data frames for any incongruities.
...
From the internal structure information, type of start_station_id and end_station_id were integer, so next step was to convert type of integer into character, in order to merge data correctly and ensure its consistency.
...
Since the business task focused on "How do annual members and casual riders use Cyclistic bikes differently?", I needed to make sure the categories, as well as their amounts, under variables rideable_type and member_casual.
...
Based on results, there were two types of users under variable member_casual: 1366575 for casual users, and 2175108 for annual members. There were three bike types under variable rideable_type: 70616 for classic_bike, 2966322 for docked_bike, and 504745 for electric_bike.
...
For the variable ride_length, it should be greater than 0 and should not be in a negative form. Also, I have removed those with ride_length greater than one day (1440 minutes). 
...
There were 11126 rows that ride_length was equal to 0 or smaller than 0, and there were 2863 rows that ride_length was equal or greater than one day. All of these rows were being removed. 
...
I wanted to compare average ride time by the each day for annual members and casual users, but first, looking at the data frame, day_of_week were out of correct of order and I would assign the actual day to the number.
...
* Casual users mostly use the bike-share service on the weekends, but less often during the weekdays. Number of rides from casual users on the weekdays was only half of number on the weekends. 
...
* Set a duration milestone and create cash-back or credit-bonus membership program to convert casual users into members because they have a longer average duration than current members. 

# LIMITATION / FUTURE IMPROVEMENT

* There were 11126 rows that ride duration was equal to or less than 0 minutes, and 2863 rows that ride duration was greater than one day. All of these were removed and we should figure out     - whether technical errors have occurred in the system (e.g. negative ride duration records)
    - and the possibility of users rent Cyclistic over a day (e.g ride duration over 1440 minutes.)
    
* There were huge number of rows that the station names were missing while the latitude and longitude information recorded. I didn't remove or drop these rows, but I think the missing station name could be fixed by comparing latitude and longitude information from other rows. 
