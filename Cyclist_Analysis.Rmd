...
Data has coverage over the period from January 2020 to December 2020. The data was downloaded and unzipped as 10- .csv files, one presented the first quarter of 2020 and the rest - 
one for each month of the range. Original copies and 10 .csv files were saved as separate folders. I opened .csv files in Excel and saved them as Excel Workbook files, so I can perform analysis later. All files were saved in separate folders (.zip / .csv / .xls) properly. 

Notes: This is a public data that can be used to explore how different customer types are using Cyclistic bikes. 
But data-privacy issues prohibit users from using riders' personally identifiable information. This means no one will b able to connect pass purchases to credit card numbers to determine if casual riders live in Cyclistic service area or if they have purchased multiple single passes.
...
I didn't remove or drop these rows, but I think the missing station name could be fixed by comparing latitude and longitude information from other rows. 
